...
PATTABLE        EQU     $0000             ; 768 x 8                          : $0000 - $17FF ($1800)
SPRPATTABLE     EQU     $1800             ; up to 32 sprites; 32 bytes each  : $0000 - $07FF (up to 2K)
COLORTABLE      EQU     $2000             ;                                  ; $2000 - $37FF ($1800)
NAMETABLE       EQU     $3800             ; 768 x 1                          : $3800 - $3AFF ($0300)
SPRATTABLE      EQU     $3B00             ; 32 x 4 bytes                     : $3B00 - $3B7F ($0080)

* VDP register settings for graphics mode II
vdp_regs       fcb      $00               ; graphics mode II
               fcb      $82               ; graphics mode II, 16K, display off, ints off
               fcb      NAMETABLE/$0400
               fcb      COLORTABLE/$0040
               fcb      PATTABLE/$0800
               fcb      SPRATTABLE/$0080
               fcb      SPRPATTABLE/$0800
               fcb      $F1               ; white text, black background
*
* Month names
MonTable       fcs      '???'
               fcs      'January'
...
*
* I/O mapping for VDP
VDP            equ      $EF80             ; TMS9918A Video Display Processor
VDP_VRAM       equ      VDP+0             ; used for VRAM reads/writes
VDP_REG        equ      VDP+1             ; control registers/address latch
VBANK_LOWER    equ      VDP+$4            ; read to select lower 16K of VRAM
VBANK_UPPER    equ      VDP+$C            ; read to select upper 16K of VRAM
VRAM           equ      $4000             ; high bits of VRAM address
...
*
               lbsr     vdp_init          ; Initialise the VDP
...
* destroys:     none
vdp_clear       pshs    d,x
                ldd     #$0000           ; write address, start at $0000
                lbsr    vdp_swaddr     ; Set up the destination VRAM address
...
* returns:	none
* destroys:
vdp_init        leax    vdp_regs,pcr      ; point X to register settings
                bsr     vdp_sregs
...
vdp_in2         bsr     copy_patterns
                leax    -1,x
                bne     vdp_in2           ; Repeat until all banks copied across
* set up the colour tabble for the patterns
                ldd     #COLORTABLE
                lbsr    vdp_swaddr
                lda     #$C1             ; text colour is green, background is black
...
copy_patterns   pshs    x,y
                bsr     vdp_swaddr     ; set up the VRAM address
...
* destroys:     none
vdp_sregs       pshs    d,x
                ldb     #$80             ; B holds register number, start at 0
vdp_set1        lda     ,x+               ; A holds register value
...
vdp_display_on  pshs    d
                ldd     #$C281     ; 0b11000010 （16K/enable/noint/gfx2/16pix sprite/x1 0b10000001 (register 1)
                sta     VDP_REG
                lbsr    vdp_wait
                stb     VDP_REG
                lbsr    vdp_wait
