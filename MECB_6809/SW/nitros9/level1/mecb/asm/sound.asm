*
* Table of Sound Byte Indexes into the Frequency Table for each of the
* 3 SN76489 Tone Generators
* What follow is Kurt Woloch's original notes:
* Here we store the melody data* 255 means pause,254 means back to start*
* all other are indexes into the frequency table
* This means that a byte value of 0 plays about the lowest possible "C" note
* (actually,18 cents below that,which is 64,73 Hz).
* A value of 12 plays the "C" one octave above it and so on... the highest
* possible note is a A#2 (described as H2 in the table above)	
* since I've encoded 36 note steps. I've often given the notes as "5+12+x" or
* something like that to somehow simulate the octaves and notes in that.
* Yes,I know,I could also have defined constants like "C#1" for the note values,
* but I didn't feel like that.
* I've added 5 at the start because the lowest note played is a G (relative to the
* main key of the melody,which in this case is actually F major,
* so that that note is acually a "C". Yes,I know it's confusing,but this is how
* I perceive and memorize music).
* Each line holds the an 1/8 note played on all three sound generators,each group
* of 8 lines thus is a measure. 
* The spaces of two lines denote the boundaries of different "parts" of the melody.
stable   FCB 0,12+7,12+4	*Ru-
