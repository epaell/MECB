********************************************************************
* Testpia1 - Write data to Prototype PIA port B
*
* $Id$
*
...
; Setup PIA Port B for LED output
               lda      #0x00             ; Select DDR Register B
               sta      PIA_CTLB          ; 
               lda      #0xFF             ; Set Port B as all outputs
               sta      PIA_PRTB          ; DDR B register write
               lda      #0x04             ; Select Port B Data Register
               sta      PIA_CTLB
               clrb                       ; set direction of shift towards left
               lda      #0x01             ; start with LED at LSB
;
loop           sta      PIA_PRTB
               tstb                       ; check scan direction
               bne      scanr
               lsla                       ; shift current bit to the left
               cmpa     #0x80             ; check if reached end
               bne      wait
               ldb      #0x01             ; change direction
               bra      wait
;
scanr          lsra                       ; shift current bit to the right
               cmpa     #0x01             ; check if at the edge
