...
*]]] Wildbits port
                  ELSE
                  IFNE    mecb ; begin conditional assembly for mecb
*[[[ MECB port
                    ldx       #Bt.Start ; load X from #Bt.Start
                    ldy       #Bt.Start+Bt.Size-1 ; load Y from #Bt.Start+Bt.Size-1
*]]] MECB port
                  ELSE
*[[[ NOT(ATARI LIBER809 port | CORSHAM port | Wildbits port | mecb port)
...
*]]] CORSHAM port
                  ...
                  IFNE    mecb ; begin conditional assembly for mecb
*[[[ MECB port
* MECB needs $0000-$04FF and $E000-$FFFF reserved.
* 0000-00ff ,x %10000000 
* 0100-01ff ,x %01000000 
* 0200-02ff ,x %00100000 
* 0300-03ff ,x %00010000 
* 0400-04ff ,x %00001000
*
* 
* e000-e0ff $1c,x %10000000 ** where IO is
* e100-e1ff $1c,x %01000000 
* e200-e2ff $1c,x %00100000 
* e300-e3ff $1c,x %00010000 
* e400-e4ff $1c,x %00001000 
* e500-e5ff $1c,x %00000100 
* e600-e6ff $1c,x %00000010 
* e700-e7ff $1c,x %00000010 
* e800-e8ff $1d,x %10000000
* e900-e9ff $1d,x %01000000 
* ea00-eaff $1d,x %00100000 
* eb00-ebff $1d,x %00010000 
* ec00-ecff $1d,x %00001000 
* ed00-edff $1d,x %00000100 
* ee00-eeff $1d,x %00000010 
* ef00-efff $1d,x %00000001
*
* f000-f7ff $1e
* f800-ffff $1f
*
                    ...
                    stb       ,x         ; mark $0000-$04FF as allocated
                    ldb       #%11111111 ; load B from #%11111111
                    stb       $1C,x      ; mark $E000-$E0FF I/O area as allocated, rest is ROM
                    stb       $1D,x      ; mark $E800-$EFFF ROM
                    stb       $1E,x      ; mark $F000-$F7FF I/O area as allocated, rest is ROM
                    stb       $1F,x      ; mark $F800-$FFFF ROM
*]]] MECB port
                  ELSE
*[[[ NOT(ATARI LIBER809 port | CORSHAM port | MECB port)
...
EOMTop              equ       *         ; define assembler symbol
                  ...
                  IFEQ    corsham+wildbits+mecb ; begin conditional assembly for corsham+wildbits+mecb
*[[[ NOT(CORSHAM port | Wildbits port | MECB port)
                    emod
...
*]]] ATARI LIBER809 port
                  ...
                  IFNE    corsham+wildbits+MECB ; begin conditional assembly for corsham+wildbits+mecb
*[[[ CORSHAM port | Wildbits port | MECB port
                    emod
eom                 equ       *         ; define assembler symbol
*]]] CORSHAM port | Wildbits port | MECB port
                  ENDC
