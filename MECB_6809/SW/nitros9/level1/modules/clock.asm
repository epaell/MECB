...
* Check if subroutine module has already been linked
                    ...
                    ifne      mecb
* Digiccool MECB 6809 board -- uses the 6840 PTM as a clock source
...
SetTimer
                    orcc      #IntMasks
                    ldd       #$0142              tell PTM to enable interrupts
                    sta       PTMCR2
                    stb       PTMCR13 
                    ldd       #$9c3f              set for 50 Hz
                    std       PTMT1MSB
                    puls      cc,d,pc
...
ClearInt            tst       PIA0Base+2          clear clock interrupt by reading register
                    ...
                    ifne      mecb
                    lda       PTMSR
                    anda      #$81                ; Mask off †imer 1 interrupt information
                    cmpa      #$81                ; Check if interrupt was from timer 1
                    beq       ClearInt            ; it's a clock interrupt -- clear it
                    jmp       [>D.SvcIRQ]         ; else service other possible IRQ
ClearInt            ldd       PTMT1MSB            ; clear timer interrupt flag
...
*
; epaell begin - don't read time until interrupt handling fixed
                    ldx       <D.Clock2           get entry point to Clock2
                    beq       L0079               branch if it doesn't exist
