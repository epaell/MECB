; Combined ASSIST09; Disassembly and Extended BASIC
; adapted from Jeff Tranter's 6809 SBC version:
...
; with minor changes to code to make it compatible with asm6809:
...
;   $A000 ACIA
; $C000-FFFF  ROM
;   $C000-$E3D7 Extended basic
;   $E400-$DFCF Disassembler
;   $F000-$F6E4 Trace
;   $F800-$FFFE ASSIST09
...
checkdest1
        ...
        lda     1,x             ; Get operand byte again
        anda    #%11110000      ; Mask source bits
        lsra                     ; Shift into low nybble
        lsra
        lsra
        lsra                     ; And fall thru to code below
...
RAMOFS  EQU     -$3900          ; ROM OFFSET TO RAM WORK PAGE ; original -$8800 sets RAM at $7000-$70FF
                                ; Set to -$3900 from RAM at top of $BFFF
ROMSIZ  EQU     2048            ; ROM SIZE
ROM2OF  EQU     ROMBEG-ROMSIZ   ; START OF EXTENSION ROM
ACIA    EQU     UART            ; DEFAULT ACIA ADDRESS
