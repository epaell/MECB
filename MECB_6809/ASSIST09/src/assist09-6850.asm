...
_PTM    EQU     52              ; PTM ADDRESS (ELENC END Changed "." to "_" to enable compilation)
...
* Fill from start of ROM ($F000) to starting location of ROM code
*  ($F800) with all ones.
...
INNECH  lda     ACIA            ; FETCH PORT STATUS
        bita    #1              ; TEST READY BIT, RDRF ?
        beq     INNECH          ; IF NOT RDY, THEN TRY AGAIN
        lda     ACIA+1          ; FETCH CHAR
        rts
;
INPUT   bsr     INNECH
;
...
;
SdBoot2 pdata1  dbmsg           ; print prompt
...
;
BootFCB fcb     0               ; drive 0
        fcb     0               ; track 0
        fcb     0               ; sector 0
        fcb     0               ; sectors per track
        fdb     BOOT_ADDR       ; buffer address
;
...
BLD3    LDA     ,Y+             ; LOAD NEXT BYTE
        ...
        CMPX    ,Y++            ; ? EXTENDED ROM HERE (ELENC changed from CPX to allow compilation)
...
ZMONTR  STS     <RSTACK         ; SAVE FOR BAD STACK RECOVERY
        ...
        JSR     xParInit        ; epaell - awful direct call to initialise PIA-SD interface
...
* A,X VOLATILE
CION   EQU      *
COON   LDA      #$03            ; RESET ACIA CODE (ELENC changed from #13 for MECB)
       LDX      <VECTAB+_ACIA   ; LOAD ACIA ADDRESS
       STA      ,X              ; STORE INTO STATUS REGISTER
       LDA      #$51            ; SET CONTROL (ELENC changed from $15 for MECB)
       STA      ,X              ; REGISTER UP
