...
# Note that Forth only works with the cpm15k bios as it occupies TPA space starting from $0500 (which would overwrite BIOS).
for disk in ["cpm80","basic","tp","tp3","ws","pli86", "V1-11", "drc86", "drc86111", "cbasic86", "CB86-719","f77-86"]:
    image = f"{disk.upper()}.IMG"
    cpmimage = f"{disk.upper()}.CPM"
    ...
    os.system(f"cpmcp -T raw -f {format}-{partition} {image} {disk}/* 0:")
    os.system(f"mv {image} {cpmimage}")
