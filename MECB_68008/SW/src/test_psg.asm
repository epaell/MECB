...
;
               ...
               move.b   #$00,d0
               move.b   #$0f,d1
               bsr      psg_atten                  ; Start with loudest
               move.b   #$01,d0
               bsr      psg_atten
               move.b   #$02,d0
               bsr      psg_atten
               move.w   #$0100,d1
               move.b   #$00,d0
               bsr      psg_freq                   ; Start a sound on channel0
               move.w   #$00E4,d1
               move.b   #$01,d0
               bsr      psg_freq                   ; Start a sound on channel1
               move.w   #$00CB,d1
               move.b   #$02,d0
               bsr      psg_freq                   ; Start a sound on channel2
               move.b   #$0F,d1
loop           move.b   #0,d0
               bsr      psg_atten                  ; Update the volume
               move.b   #1,d0
               bsr      psg_atten
               move.b   #2,d0
               bsr      psg_atten
               tst.b    d1
               beq      test_end                   ; if zero volume then exit
               sub.b    #1,d1                      ; lower it by one
               move.l   #8000,d2
loop1          sub.l    #1,d2
               bne      loop1
               bra      loop
...
isr_return     movem.l  (a7)+,d0-d2/a0-a2 ; restore registers
               rte
;
psg_init       move.b   #$22,PIA1CTLB     ; Setup PIA Port B for Sound ouput
...
psg_stop       movem.l  d0-d1,-(a7) ; Save registers
               ...
               bsr      psg_atten
               add.b    #1,d0       ; Turn off channel 4
               bsr      psg_atten
               movem.l  (a7)+,d0-d1 ; Restore registers
               rts
;
; Function : Set channel attenuator for PSG
; Parameters:  d0 - channel (0-3, 3=noise)
;              d1 - level (0-15, 15=off)
psg_atten      movem.l  d0/d1,-(a7)    ; save registers
...
;              d1 - frequency (0-1023)
psg_freq       movem.l  d0/d2,-(a7)    ; save registers
               lsl.b    #5,d0          ; move channel number to bits 5 and 6
