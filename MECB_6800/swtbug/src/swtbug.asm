...
SCRATCH  EQU   $7F00
IRQ      EQU   SCRATCH+$00    IRQ POINTER
BEGA     EQU   SCRATCH+$02    BEGINNING ADDR PNCH
ENDA     EQU   SCRATCH+$04    ENDING ADDR PNCH
NMI      EQU   SCRATCH+$06    NMI INTERRUPT VECTOR
SP       EQU   SCRATCH+$08    S HIGH
SPLOW    EQU   SCRATCH+$09    S LOW
PORADD   EQU   SCRATCH+$0A    PORT ADDRESS
PORECH   EQU   SCRATCH+$0C    ECHO ON/OFF FLAG
XHI      EQU   SCRATCH+$0D    XREG HIGH
XLOW     EQU   SCRATCH+$0E    XREG LOW
CKSM     EQU   SCRATCH+$0F    CHECKSUM
XTEMP    EQU   SCRATCH+$10    X-REG TEMP STGE
SWIJMP   EQU   SCRATCH+$12    SWI JUMP VECTOR
BKPT     EQU   SCRATCH+$14    BREAKPOINT ADDRESS
BKLST    EQU   SCRATCH+$16    BREAKPOINT DATA
STACK    EQU   SCRATCH+$42    SWTBUG STACK
TW       EQU   SCRATCH+$44    TEMPORARY STORAGE
TEMP     EQU   SCRATCH+$46    TEMPORARY STORAGE
BYTECT   EQU   SCRATCH+$47    BYTECT AND MCONT TEMP.
CTLPOR   EQU   $8008          CONTROL PORT ADDRESS
...
CURSOR   dc.b  $10,$16,4      CT-1024 CURSOR CONTROL
...
START    LDS   #STACK
         BRA   AL1            BRANCH FOR ADDRESS COMPATIBILITY
...
*INITIALIZE ACIA
         ...
         STAA  0,X
         LDAA  #$51           EPAELL Changed from $11
         STAA  0,X
         BRA   CONTRL
...
S9       dc.b  'S','9',4      END OF TAPE

***************************************************
MTAPE1   dc.b  $D,$A,$15,0,0,0,'S','1',4    PUNCH FORMAT
...
*         BNE   IN1            INPUT FROM PIA IF NOT
         LDAA  #$51           RECONFIG FOR 8 BIT, 1 SB
         STAA  0,X
ACIAIN   LDAA  0,X
         ASRA
...
TABLE    dc.b  'G'            GOTO
         ...
         dc.b  'Z'            GOTO PROM
         dc.w  PROM
         dc.b  'M'            MEMORY EXAM AND CHANGE
         dc.w  CHANGE
         dc.b  'F'            BYTE SEARCH
         dc.w  SEARCH
         dc.b  'R'            REGISTER DUMP
         dc.w  PRINT
         dc.b  'J'            JUMP
         dc.w  JUMP
         dc.b  'C'            CLEAR SCREEN
         ...
         dc.b  'B'            BREAKPOINT
         dc.w  BREAK
         dc.b  'O'            OPTIONAL PORT
         dc.w  OPTL
TABLE1   dc.b  'P'            ASCII PUNCH
         dc.w  PUNCH1
         dc.b  'L'            ASCII LOAD
         dc.w  LOAD
TABEND   dc.b  'E'            END OF TAPE
         dc.w  PNCHS9

         include  'trek.asm'

         ORG   $FFF8
         dc.w  IRQV           IRQ VECTOR
         dc.w  SFE            SOFTWARE INTERRUPT
         dc.w  NMIV           NMI VECTOR
