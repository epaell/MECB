...
load0b:
         ...
         ldb   6,x
;         lda   #$02            ; For QLOAD need this to load FLEX.COR
;         ldb   #36
         sta   SCTBUF
         stb   SCTBUF+1
...
; This bit checks to see if overwriting the driver
         ...
         ldb   ladr+1
         cmpa  #$b3
         blo   store           ; OK to store in <$b300
         bne   lc1             ; if not $b3 check other locations
         cmpb  #$90
         blo   store           ; OK to store in <$b390
         cmpb  #$fd
         blo   skip            ; skip anything between $b390 and $b3fc
         bra   store           ; otherwise store
lc1      cmpa  #$be
         bhs   skip            ; skip anything >=$be00
store:   lda   ltempa
         ldb   ltempb
...
;         lda   0,x             ; Get forward link (from previous read - track)
;         beq   go              ; if zero, file is loaded
;         ldb   1,x             ; else, get sector
;
;         psha
;         lda   #'.'
;         jsr   foutch
...
;
         org   $b390
;
; $b390-b3e4 set aside for console drivers
;
...
;
; $bE00-$bfff set aside for drivers
