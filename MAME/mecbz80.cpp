...
class mecbz80_state : public driver_device
{
public:
	mecbz80_state(const machine_config &mconfig, device_type type, const char *tag)
	...
	void mecbz80(machine_config &config);

protected:
	// address maps for program memory and io memory
	void mecbz80_mem(address_map &map);
	void mecbz80_io(address_map &map);
...
void mecbz80_state::mecbz80_mem(address_map &map)
{
	map(0x0000, 0x1fff).rom();
	map(0x2000, 0xffff).ram();
}

void mecbz80_state::mecbz80_io(address_map &map)
{
...
void mecbz80_state::mecbz80(machine_config &config)
{
	/* basic machine hardware */

	// Configure member Z80 (via m_maincpu)
	Z80(config, m_maincpu, XTAL(7'372'800));
	m_maincpu->set_addrmap(AS_PROGRAM, &mecbz80_state::mecbz80_mem);
	m_maincpu->set_addrmap(AS_IO, &mecbz80_state::mecbz80_io);
...
COMP( 2024, mecbz80,          0,        0,        mecbz80,         0,             mecbz80_state,      empty_init,    "Digicool Things",   "MECB Z-80",  MACHINE_NO_SOUND_HW )
